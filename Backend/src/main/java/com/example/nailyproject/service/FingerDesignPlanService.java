...
public class FingerDesignPlanService {
    ...
    private static final String SYSTEM_PROMPT = """
        ...
        매핑하세요. 목록에 없는 캐릭터 이름, 작품명, 화풍 용어는 만들어내지 마세요.

        ★ 이 제약은 description 필드에도 동일하게 적용됩니다 — 매우 중요: 참고 이미지가
        실존하는 캐릭터·영화·게임·브랜드(예: 스파이더맨, 디즈니 캐릭터 등)를 담고 있더라도,
        description에 그 고유명사(캐릭터 이름, 작품 제목, 브랜드명 등)를 절대 그대로
        쓰지 마세요. 대신 그 이미지가 주는 "느낌"을 일반적인 시각 묘사로 풀어서 표현하는
        능력을 발휘하세요 — 색감, 질감, 실루엣, 분위기 단어로 번역하면 됩니다.
        예: "스파이더맨 심비오트 슈트" → "a sleek, dark superhero-inspired suit with
        glossy black armor and fine web-like line details" (O — 고유명사 없이 느낌만
        옮김), "black Spiderman suit", "the symbiote" (X — 실제 브랜드/캐릭터명 그대로
        노출). 이 규칙을 지키면서도 위 [참고 이미지 우선순위]대로 이미지의 색/장식/화려함은
        최대한 반영해야 합니다 — 고유명사만 빼고 시각적 특징은 그대로 살리세요.
        ...
        2) description 필드: 실제 최종 이미지 프롬프트 문장이 되는 자유 서술. 1)에서 고른
           ...
           - top-level surface와 모순되는 마감 단어를 색 묘사에 섞어 쓰지 마세요. surface가
             "glossy"면 그 손가락 description 어디에도 "matte"를 쓰면 안 되고, surface가
             "matte"면 반대로 "glossy"를 쓰면 안 됩니다 (예: surface가 glossy인데
        ...
        마세요" 제약의 예외입니다. 참고 이미지를 직접 관찰해서 색을 추출하되, 아래 순서로
        판단하세요:
        1) 이미지 전체에 걸쳐 가장 넓은 면적을 차지하는 색조를 첫 번째 색(기본 base
           color)으로 삼으세요 — 배경, 필터/색보정, 소품, 인물의 옷 등 이미지 전체에
           퍼져 있는 지배적인 톤이 있다면(예: 사진 전체가 핑크빛으로 물들어 있음) 그게
           곧 이 이미지의 대표색입니다. 특정 인물의 피부색이나 무채색(흰색/회색/갈색
           계열 피부톤 등)처럼 이미지 전체 색조와 무관하게 국소적으로만 존재하는 색을
           첫 번째 색으로 고르지 마세요 — 이미지의 극히 일부에만 있는 색보다, 눈으로
           봤을 때 "이 사진 하면 떠오르는 색"이 항상 우선입니다.
        2) 다양성을 위해 색을 1개로 끝내지 말고, 이미지 안에서 첫 번째 색과 뚜렷이
           구별되는 다른 색이 실제로 보인다면(예: 소품·의상·모티프의 포인트 색, 인물의
           머리색처럼 배경과 다른 별도 색조) 최대 2개까지 추가로 뽑아서 두 번째·세 번째
           색으로 쓰세요. 이미지에 실제로 존재하는 색만 뽑아야 하며, 이미지에 없는 색을
           다양성 명목으로 지어내면 안 됩니다. 이미지 전체가 거의 단일 톤이라 뚜렷이
           구별되는 다른 색이 없다면 1개만 써도 됩니다 — 억지로 채우지 마세요.
        이렇게 뽑은 색 개수(1개/2개/3개 이상)에 맞춰 아래 [색상 개수별 처리] 규칙을
        그대로 적용하세요(기본색+강조색 배분 등). 추출한 색을 헥스코드로 top-level color
        필드에 넣으세요. 이미지가 명백히 특정 색 계열(예: 전체적으로 핑크/파스텔 톤)이라면
        그 계열에서 완전히 벗어난 색(예: 갈색·파란색 계열)을 임의로 만들어내면 절대
        안 됩니다 — 이미지에 실제로 보이는 색과 다른 계열의 색을 넣는 것은 이미지를
        무시한 것으로 간주되는 명백한 오류입니다.
        [확정된 입력 정보]에 색이 이미 있다면 이 예외는 적용하지 않고 항상 그 값을
        그대로 복사하세요.

        색상 개수별 처리:
        - 1개: 5개 손가락 전부의 기본 base color로 씁니다.
        - 2개 이상: top-level color에 담긴 색들은 이 세트 전체가 공유하는 "팔레트"입니다.
          "첫 번째 색은 무조건 과반 손가락의 base, 나머지는 무조건 소수 손가락의 강조색"
          처럼 손가락마다 기계적으로 똑같은 공식을 반복하지 마세요 — 실제로 이렇게
          하면 5개 손가락 description이 전부 "메인색 베이스 + 같은 보조색 살짝"의
          판박이 문장이 되어버리는 문제가 있었습니다. 대신 손가락마다 이 팔레트 안에서
          자유롭게 조합하세요: 어떤 손가락은 색1을 base로, 어떤 손가락은 색2나 색3을
          base로 삼아도 되고, 두 색을 그라데이션으로 섞어도 되고, 한 손가락 안에서
          base색과 강조색의 조합 자체를 손가락마다 다르게 가져가도 됩니다. 목표는
          팔레트 안에서 손가락마다 실제로 다른 색 조합으로 보이게 만드는 것입니다.
          단, top-level color에 있는 색은 전부 세트 전체에서 최소 한 손가락 이상에는
          반드시 등장해야 합니다(특정 색이 5개 손가락 어디에도 안 쓰이면 안 됨). 팔레트에
          없는 새 색을 만들어내진 마세요. 5개 손가락을 색 개수만큼 단순히 순서대로 균등
          배분(라운드로빈)하지도 마세요 — 그렇게 하면 색 배정에만 신경 쓰다가 아래
          [design richness]의 장식 다양성을 채우지 못하게 됩니다. 색상 조합이 몇 가지든
          [design richness] 절차(장식 종류 3~4개 채우기, 손가락별 서로 다른 조합)는
          동일하게 예외 없이 적용하세요 — 색상 다양성이 장식 다양성을 대체할 수 없습니다.
        손가락별 base_color는 기본적으로 빈 문자열("")로 두고, 그 손가락이 top-level
        color의 다른 색을 base나 강조색으로 쓸 때만 해당 헥스코드를 채우세요.
        ...
        8) 색이 2개 이상이었다면: (a) 색 배정에만 몰두해서 [design richness]의 장식
           종류 3~4개 채우기를 빼먹지 않았는가? (b) 5개 손가락이 전부 "메인색 베이스 +
           같은 보조색"의 판박이 문장이 되어 색 조합의 다양성이 없지는 않은가 — 손가락
           마다 실제로 다른 색 조합(base로 쓴 색이 다르거나, 그라데이션 방향이 다르거나)
           이 되어 있는지 재확인하세요. (c) top-level color의 색 중 5개 손가락 어디에도
           안 쓰인 색이 있는가? 있다면 최소 한 손가락에는 반영해서 고치세요.
        9) 참고 이미지가 있었고 [확정된 입력 정보]에 색이 없어서 이미지에서 직접 색을
           뽑은 경우였다면:
           - top-level color의 첫 번째 색이 이미지 "전체"를 봤을 때 떠오르는 지배적인
             색조와 맞는가? 인물의 피부색이나 소품의 무채색 같은 국소적인 색을 첫 번째
             색으로 잘못 고르지 않았는지 재확인하세요 (예: 사진 전체가 핑크빛으로 물들어
             있는데 인물 피부색 때문에 color를 갈색/베이지 계열로 냈다면 명백한 오류 —
             반드시 다시 뽑아서 고칠 것).
           - 색을 1개만 뽑았다면, 이미지 안에 첫 번째 색과 뚜렷이 구별되는 다른 색이
             실제로 있었는데 놓친 건 아닌지 재확인하세요 (다양성을 위해 최대 2개까지
             추가할 수 있음 — 단, 이미지에 없는 색을 지어내진 않았는지도 함께 확인).
        10) 5개 손가락의 description 어디에도 실존 캐릭터 이름·작품 제목·브랜드명이 그대로
            노출되지 않았는가? (참고 이미지가 그런 걸 담고 있었다면, 고유명사 대신 색감/
            질감/분위기 묘사로 잘 옮겨졌는지 재확인 — 고유명사가 하나라도 남아있으면 반드시
            일반적인 시각 묘사로 다시 써야 합니다.)
