...
public class FingerDesignPlanService {
    ...
    private static final String SYSTEM_PROMPT = """
        ...
        판단하세요:
        1) 이미지 전체에 걸쳐 가장 넓은 면적을 차지하는 색조를 최우선으로 삼으세요 —
           배경, 필터/색보정, 소품, 인물의 옷 등 이미지 전체에 퍼져 있는 지배적인 톤이
           있다면(예: 사진 전체가 핑크빛으로 물들어 있음) 그게 곧 이 이미지의 색이므로
           그대로 쓰세요. 특정 인물의 피부색이나 무채색(흰색/회색/갈색 계열 피부톤 등)처럼
           이미지 전체 색조와 무관하게 국소적으로만 존재하는 색을 대표색으로 고르지
           마세요 — 이미지의 극히 일부에만 있는 색보다, 눈으로 봤을 때 "이 사진 하면
           떠오르는 색"이 항상 우선입니다.
        2) 이미지 전체에 지배적인 톤이 뚜렷하지 않다면(여러 색이 고르게 섞여 있음),
           그때는 주요 피사체·소품의 색을 참고해서 1~2개를 고르세요.
        추출한 색을 헥스코드로 top-level color 필드에 넣으세요. 이미지가 명백히 특정 색
        계열(예: 전체적으로 핑크/파스텔 톤)이라면 그 계열에서 벗어난 색(예: 갈색·파란색
        계열)을 임의로 만들어내면 절대 안 됩니다 — 이미지에 실제로 보이는 색과 다른
        계열의 색을 넣는 것은 이미지를 무시한 것으로 간주되는 명백한 오류입니다.
        ...
        9) 참고 이미지가 있었고 [확정된 입력 정보]에 색이 없어서 이미지에서 직접 색을
           뽑은 경우였다면, top-level color(그리고 description의 색 표현)가 이미지
           "전체"를 봤을 때 떠오르는 지배적인 색조와 맞는가? 인물의 피부색이나 소품의
           무채색 같은 국소적인 색을 대표색으로 잘못 고르지 않았는지 재확인하세요 (예:
           사진 전체가 핑크빛으로 물들어 있는데 인물 피부색 때문에 color를 갈색/베이지
           계열로 냈다면 명백한 오류 — 반드시 다시 뽑아서 고칠 것).
